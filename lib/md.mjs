...
// rendered page (content and templates alike). When analytics is on, also
// tag them so Umami records an "outbound" event with the destination URL.
...
export function externalLinks(html) {
  return html.replace(rATag, (tag, href) => {
    ...
    const track = config.umamiWebsiteId
      ? `data-umami-event="outbound" data-umami-event-url="${href}" `
      : "";
    if (/rel=/i.test(tag)) {
      return tag
        .replace(/rel=["']([^<>"']*)["']/i, (m, rel) =>
        ...
        .replace('href=', `${track}target="_blank" href=`);
    }
    return tag.replace('href=', `${track}target="_blank" rel="noopener" href=`);
  });
}
