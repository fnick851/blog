...
// Umami tracker. Loaded through the /stats rewrites in vercel.json so the
// page only ever talks to its own origin; data-domains keeps `npm run serve`
// and Vercel preview deployments out of the numbers.
function analytics() {
  if (!config.umamiWebsiteId) return "";
  const host = new URL(config.url).hostname;
  return `
    <script defer src="/stats/script.js" data-website-id="${config.umamiWebsiteId}" data-host-url="/stats" data-domains="${host}"></script>`;
}

// `nav: false` drops the menu links: the home page's body already carries
// the archive link, and the archive page is the link's own destination.
function header({ nav = true } = {}) {
  const navItems = nav
    ? Object.entries(config.nav)
...
// `options` is passed to header().
export function layout(title, main, options) {
  return `<!DOCTYPE html>
...
// `postCount` feeds the one body link to the archive; without it the home
// page reads as a profile card and the header's small "posts" link is easy
// to miss.
export function profilePage(descriptionHTML, postCount) {
  const [navLabel, navHref] = Object.entries(config.nav)[0];
  ...
  <div class="intro">
    ...
    <div class="posts-link">
      <a href="${navHref}"><span class="arrow">➜</span>${postCount} ${navLabel}</a>
    </div>
  </div>
</div>
